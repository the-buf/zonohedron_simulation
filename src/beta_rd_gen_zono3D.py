...
n_prime = int(
    n / 10
)  # n_prime et n_second sont calculés pour n < 3600 (état actuel des choses)
...
def generateur_primitif(j):
    ...
    if dimension > liste_phi[n] / liste_gene[n]:
        while math.gcd(p, math.gcd(q - p, n - q)) != 1 and (
            p == 0 or q - p == 0 or n - q == 0
        ):
        ...
        if coin1 == 0:
            ...
            q = p - n
        if coin2 == 0:
            return (0.0, float(p), float(q))
        elif coin2 == 1:
            return (float(p), 0.0, float(q))
        else:
            return (float(p), float(q), 0.0)
