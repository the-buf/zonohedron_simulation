...
K.append(
    K[len(K) - 1]
    + (n_prime * x ** (n_prime - 1) - (n_prime - 1) * x**n_prime) / (1 - x) ** 2
)  ## ce calcul est trop bizarre
...
for m in range(1, n_prime):
    N.append(
        [A[m][i] / A[m][len(A[m]) - 1] for i in range(1, len(A[m]))]
    )  ########pourquoi m et pas i ??
...
def generateur_zonogone():
    ...
    for i in range(1, k + 2):
        ...
        while (
            a == 0 or i > k + 1
        ):  # je ne comprends pas cette boucle (mais je ne comprends pas la différence dans l'algo)
...
for j in range(nb_zonogones):
    ...
    ax[j].plot(
        x, y, color="#6699cc", alpha=0.7, linewidth=1, solid_capstyle="round", zorder=2
    )
...
y = [((2 * x[i]) ** (1 / 2) - x[i]) + 0.5 for i in range(len(x))]
x_1 = [1 - alph * ((39 - i) * (40 - i)) / 2 for i in range(40)]
y_1 = [y[len(y) - 1 - i] for i in range(len(y))]
X = x + x_1
Y = y + y_1
ay = fig.add_subplot(111)
ay.plot(X, Y, color="black", alpha=0.7, linewidth=1, solid_capstyle="round", zorder=2)
